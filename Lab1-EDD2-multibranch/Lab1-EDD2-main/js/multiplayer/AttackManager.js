...
class AttackManager {
    ...
    // Function to apply an attack via sending data.
    applyAttack(attackKey, payload, graph, renderer, ui) {
        const def = this.ATTACKS[attackKey];
        if (!def) {
    ...
    _applyFirewall(payload, graph, renderer, ui, duration) {
        const nodes = graph.getAllNodes().filter(n => !n.classified);
        ...
        this._activeEffects['FIREWALL'] = setTimeout(() => {
            target.firewalled = false;
            renderer.draw();
            delete this._activeEffects['FIREWALL'];
        }, duration);
    }
    ...
    _applyEdgeDelete(graph, renderer, duration) {
        ...
        if (duration) {
            this._activeEffects['EDGE_DELETE'] = setTimeout(() => {
                ...
                renderer.draw();
                delete this._activeEffects['EDGE_DELETE'];
            }, duration);
        }
    }
